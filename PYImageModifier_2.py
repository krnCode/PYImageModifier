...
for rawImage in os.listdir(rawDir):
    ...
    img_sharpen.save(modDir + '/' + rawImage[:-4] + '-sharp.png')
    img_blur = rgbImg.filter(ImageFilter.BLUR)
    img_blur.save(modDir + '/' + rawImage[:-4] + '-blur.png')
    img_smooth = rgbImg.filter(ImageFilter.SMOOTH)
    img_smooth.save(modDir + '/' + rawImage[:-4] + '-smooth.png')
    img_contour = rgbImg.filter(ImageFilter.CONTOUR)
    img_contour.save(modDir + '/' + rawImage[:-4] + '-contour.png')
    img_detail = rgbImg.filter(ImageFilter.DETAIL)
    img_detail.save(modDir + '/' + rawImage[:-4] + '-detail.png')
    img_emboss = rgbImg.filter(ImageFilter.EMBOSS)
    img_emboss.save(modDir + '/' + rawImage[:-4] + '-emboss.png')


# Print message telling that everything is done
